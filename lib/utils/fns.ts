...
export class Ref<D> {
  ...
  render(data?: D, stash?: boolean) {
    ...
    const av = async () => {
      ...
      CradovaEvent.removeEventListener("cradovamount", av);
    };
    CradovaEvent.addEventListener("cradovamount", av);
    this.effector();
    this.published = true;
    this.rendered = true;
    if (!element) {
...
export const frag = function (children: any) {
  const par = naxt.createElement("div");
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (a instanceof String) {
      par.appendChild(naxt.createTextNode(a as string));
      continue;
    }
