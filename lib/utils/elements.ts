...
import { reference, Rhoda, CradovaEvent } from "./fns";
import { createSignal } from "./createSignal";
import { Element, naxt } from "./naxt";
    ...
    for (const prop in props) {
      ...
      if (prop === "onmount" && typeof props["onmount"] === "function") {
        const avi = () => {
          ...
          CradovaEvent.removeEventListener("cradovamount", avi);
        };
        CradovaEvent.addEventListener("cradovamount", avi);
        continue;
      }
...
const cra: any = (element_initials: string) => {
  return (...initials: any[]) => {
    return makeElement(naxt.createElement(element_initials), ...initials);
  };
};
